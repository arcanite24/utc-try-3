...
{{ csrf_field() }}
    ...
    @foreach($administrador as $admin) 
		@if ($admin->id == 1) <?php $parcial_a=$admin->parcial_activo ?>
		@endif
    @endforeach
...
@if($op==false) action="{{ url('insert') }}" 
@else action="{{ url('update') }}"
	...
	<thead class="bg-success">
	   ...
	   <?php $i=0 ?>
		...
		<tr @if($op==true) 
		    ...
		    @if ($cal->id_calificaciones == $a->id_alumnos.$id_materias) value={{$cal->parcial1}} 
		    @endif
		    @endforeach></td>
			...
			@foreach($calificaciones as $cal) 
		    @if ($cal->id_calificaciones == $a->id_alumnos.$id_materias) value={{$cal->parcial2}} 
		    @endif
		    @endforeach></td>
			...
			@foreach($calificaciones as $cal) 
		    @if ($cal->id_calificaciones == $a->id_alumnos.$id_materias) value={{$cal->parcial3}} 
		    @endif
		    @endforeach></td>
